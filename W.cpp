...
//	namedWindow ("threshold" ,CV_WINDOW_AUTOSIZE ); //----------------------------- manipulating threshold
...
/*
void switchCallback(int position, void*){
	lowThreshold = position;
}*/

void PainterWidget::value(int newThres){
	lowThreshold = newThres;
}
...
//	if(detect){processImage(iplimg);detect =0;}	
  	Mat src1(iplimg);
//	src = src1;
	Mat gray, blur, edge, draw; //gray: grayscale	blur: Gaussian blur   edge: edge detect   draw: Ipl Format  
	cvtColor(src1, gray, CV_BGR2GRAY);
//	createTrackbar("HIGH","Threshold", &lowThreshold, max_lowThreshold,  switchCallback);
//	createTrackbar( "Min Threshold:", "thres", &lowThreshold, max_lowThreshold, CannyThreshold );

//	blur( gray, blur, Size(3,3) );
	GaussianBlur(gray,blur,Size(0,0), 4); //Gaussian blur, remove background
	Canny( blur, edge, lowThreshold, lowThreshold*3, 3); //Canny Edge Detect
